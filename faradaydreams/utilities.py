...
def get_polarization_and_weights(P_q, P_u, error_q, error_u):
    ...
    weights = np.std(np.abs(weights_q + 1j * weights_u))/np.abs(weights_q + 1j * weights_u) 
...
def calculate_lambda2(freq, dfreq):
    ...
    if dfreq is None:
        return lambda2, None
    dlambda2 = (c/(freq - dfreq/2.))**2 - (c/(freq + dfreq/2.))**2 #channel width in wavelength squared.
    return lambda2, dlambda2

def convolve_solution(solution, phi, sigma_phi):
    return np.convolve(solution, np.exp(-phi[::-1]**2/(2 * sigma_phi**2)),'same')
